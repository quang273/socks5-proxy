...
EOF
    ...
    # Ensure systemctl is available before trying to use it
    if command -v systemctl &> /dev/null; then
        ...
        sudo service danted restart || true # Fallback for non-systemd systems
    fi
...
# Check if running on a Google Cloud Compute Engine instance (more robust check)
# A typical GCE instance will have its name set in instance metadata service.
# Cloud Shell also has access, but we can differentiate by checking the environment variable `CLOUD_SHELL`.
# CLOUD_SHELL is typically set to 'true' in Google Cloud Shell.
if [[ "$CLOUD_SHELL" == "true" ]]; then
    echo "Script detected running in Google Cloud Shell. Starting main deployment process (project and instance creation)..."

    # Only set set -e within the subshell for project creation,
    # as project creation can fail and we want the loop to continue.
    set +e # Temporarily disable set -e for the main loop error handling
    ...
    while [ "$SUCCESSFUL_PROJECT_COUNT" -lt "$NUM_PROJECTS_TO_CREATE" ]; do
      ...
      ( # Start a subshell for each project creation attempt to allow parallel execution
        ...
        if ! gcloud projects create "$PROJECT_ID" --name="$PROJECT_ID"; then
          echo "!!! Error: Failed to create project $PROJECT_ID. Might be a duplicate ID, quota limit, or other issue. Deleting project if it partially created."
...
else # This branch executes if not in Cloud Shell, implying it's a GCE VM
